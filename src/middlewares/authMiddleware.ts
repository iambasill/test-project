...
export const authMiddleware = async(req: Request, _res: Response, next: NextFunction) => {
  ...
  try {
       decoded = jwt.verify(token, AUTH_JWT_TOKEN as string) as JwtPayload;
    ...
    if (!user || user.status !== "ACTIVE" || user.isActive === false) throw new unAuthorizedError("Access Denied")
    req.user = user
    req.token = token
    next();
}